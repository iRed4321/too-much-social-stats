...
import { Action, Conv, Tab, addDiv, MsgKind, MsgTextKind } from "./lib/common";
...
class ActionLoop {
    ...
    update() {
        switch (this.action.type) {
            case "updateData":
                this.objects.convAll.updateData(
                    this.convs.toTable(
                        this.objects.filters.get(),
                        this.objects.slider.getCurrRange()
                    )
                );
                break;

            case "clickRow":
                this.objects.convOne.setRow(
                    this.convs.get(this.objects.convAll.getSelectedRow())
                );
                break;
